...
class TradeSignalManager:
    ...
    def __init__(
        ...
        logger.info(
            f"{symbol} için TradeSignalManager başlatıldı, {len(indicators)} gösterge kullanılıyor"
        )
    ...
    def _get_signal_from_indicator(
        self, name: str, indicator: BaseIndicator
    ) -> SignalType:
        """
        ...
        if "rsi_middle_band" in name.lower():
            ...
            if hasattr(indicator, "is_buy_signal") and hasattr(
                indicator, "is_sell_signal"
            ):
    ...
    def _calculate_combined_signal(
        self, signal_counts: Dict[SignalType, int]
    ) -> SignalType:
        """
        ...
        if (
            ...
            and signal_counts[SignalType.CLOSE_LONG] >= self.min_signal_agreement
        ):
            return SignalType.CLOSE_LONG

        if (
            self.current_position == PositionType.SHORT
            and signal_counts[SignalType.CLOSE_SHORT] >= self.min_signal_agreement
        ):
            return SignalType.CLOSE_SHORT
    ...
    def _process_signal(self, signal: SignalType) -> None:
        ...
        if signal == SignalType.LONG and self.current_position == PositionType.NONE:
            ...
            and self.current_position == PositionType.LONG
        ):
            ...
            and self.current_position == PositionType.SHORT
        ):
    ...
    def _open_position(self, position_type: PositionType, price: float) -> None:
        ...
        if self.current_position != PositionType.NONE:
            logger.warning(
                f"Zaten açık bir pozisyon var: {self.current_position.value}"
            )
            return
        ...
        try:
            ...
            logger.info(
                f"{side} emri gönderiliyor: {position_size} {self.symbol} @ {price}"
            )

            # Gerçek API çağrısı
            order = self.client.create_market_order(
                symbol=self.symbol, side=side, quantity=position_size
            )
            ...
            trade_info = {
                ...
                "signals": {
                    name: signal.value for name, signal in self.last_signals.items()
                },
            }
            ...
            # Log ve bildirim
            logger.info(
                f"Pozisyon açıldı: {position_type.value} {position_size} {self.symbol} @ {price}"
            )
    ...
    def _close_position(self, price: float) -> None:
        ...
        try:
            ...
            logger.info(
                f"{side} emri gönderiliyor: {self.position_size} {self.symbol} @ {price}"
            )

            # Gerçek API çağrısı
            order = self.client.create_market_order(
                symbol=self.symbol, side=side, quantity=self.position_size
            )
            ...
            # Log ve bildirim
            logger.info(
                f"Pozisyon kapatıldı: {self.current_position.value} {self.position_size} {self.symbol}, "
    ...
    def _get_current_price(self) -> Optional[float]:
        ...
        """
        try:
            ticker = self.client._public_request(
                "GET", "/api/v3/ticker/price", {"symbol": self.symbol}
            )
