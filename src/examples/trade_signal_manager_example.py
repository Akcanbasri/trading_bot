...
from src.trading.trade_signal_manager import TradeSignalManager, SignalType, PositionType
...
def backtest_with_signal_manager(data: pd.DataFrame, symbol: str = "BTCUSDT"):
    ...
    if equity:
        ...
        profitable_trades = sum(1 for trade in trade_history if trade.get("type") == "CLOSE" and trade.get("pnl", 0) > 0)
        total_closed_trades = sum(1 for trade in trade_history if trade.get("type") == "CLOSE")
        
        if total_closed_trades > 0:
            win_rate = profitable_trades / total_closed_trades * 100
            logger.info(f"Kazanç Oranı: %{win_rate:.2f} ({profitable_trades}/{total_closed_trades})")
...
def plot_backtest_results(dates, prices, signals, positions, equity, symbol):
    ...
    # İşaretleri ekle
    long_entry_dates = [date for date, pos, prev_pos in zip(dates, positions, ['NONE'] + positions[:-1]) 
                        if pos == 'LONG' and prev_pos != 'LONG']
    long_entry_prices = [price for price, pos, prev_pos in zip(prices, positions, ['NONE'] + positions[:-1]) 
                         if pos == 'LONG' and prev_pos != 'LONG']
    
    short_entry_dates = [date for date, pos, prev_pos in zip(dates, positions, ['NONE'] + positions[:-1]) 
                         if pos == 'SHORT' and prev_pos != 'SHORT']
    short_entry_prices = [price for price, pos, prev_pos in zip(prices, positions, ['NONE'] + positions[:-1]) 
                          if pos == 'SHORT' and prev_pos != 'SHORT']
    
    long_exit_dates = [date for date, pos, prev_pos in zip(dates, positions, ['NONE'] + positions[:-1]) 
                       if pos == 'NONE' and prev_pos == 'LONG']
    long_exit_prices = [price for price, pos, prev_pos in zip(prices, positions, ['NONE'] + positions[:-1]) 
                        if pos == 'NONE' and prev_pos == 'LONG']
    
    short_exit_dates = [date for date, pos, prev_pos in zip(dates, positions, ['NONE'] + positions[:-1]) 
                        if pos == 'NONE' and prev_pos == 'SHORT']
    short_exit_prices = [price for price, pos, prev_pos in zip(prices, positions, ['NONE'] + positions[:-1]) 
                         if pos == 'NONE' and prev_pos == 'SHORT']
    
    # Long ve Short işlem noktalarını işaretle
    ax1.scatter(long_entry_dates, long_entry_prices, marker='^', color='green', s=100, label='Long Giriş')
    ax1.scatter(short_entry_dates, short_entry_prices, marker='v', color='red', s=100, label='Short Giriş')
    ax1.scatter(long_exit_dates, long_exit_prices, marker='o', color='blue', s=80, label='Long Çıkış')
    ax1.scatter(short_exit_dates, short_exit_prices, marker='o', color='orange', s=80, label='Short Çıkış')
    ...
    # Sinyaller grafiği
    signal_values = [1 if s == 'LONG' else -1 if s == 'SHORT' else 0 if s == 'NEUTRAL' 
                     else 0.5 if s == 'CLOSE_LONG' else -0.5 if s == 'CLOSE_SHORT' else 0 for s in signals]
    ...
    ax3.axhline(y=equity[0], color='gray', linestyle='--', alpha=0.5, label='Başlangıç Sermayesi')
    ax3.legend()
