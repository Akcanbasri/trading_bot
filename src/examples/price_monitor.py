...
def monitor_prices(symbol: str = "DOGEUSDT", duration_seconds: int = 60):
    ...
    try:
        ...
        logger.error(
            "Please make sure you have set up your API credentials in the .env file"
        )
        return

    logger.info(
        f"Starting price monitoring for {symbol} for {duration_seconds} seconds..."
    )
    ...
    try:
        while time.time() - start_time < duration_seconds:
            ...
            if last_price is not None:
                ...
                logger.info(
                    f"{current_time} - {symbol}: ${current_price:.8f} {direction} "
                    f"({change_percent:+.4f}%)"
                )
            else:
                logger.info(
                    f"{current_time} - {symbol}: ${current_price:.8f} (initial price)"
                )
