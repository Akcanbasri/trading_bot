...
def place_limit_orders(client: BinanceClient, symbol: str):
    ...
    price_filter = next(
        (f for f in symbol_info["filters"] if f["filterType"] == "PRICE_FILTER"), None
    )
    ...
    logger.info(
        f"Fiyat hassasiyeti: {price_precision}, Miktar hassasiyeti: {quantity_precision}"
    )
    ...
    logger.info(
        f"İşlem yapılacak miktar: {quantity} BTC (yaklaşık ${quantity * current_price:.2f})"
    )
    ...
    try:
        ...
        buy_order = client.create_limit_order(
            symbol=symbol, side="BUY", quantity=quantity, price=buy_price
        )
        ...
        sell_order = client.create_limit_order(
            symbol=symbol, side="SELL", quantity=quantity, price=sell_price
        )
        ...
        for order in open_orders:
            ...
            logger.info(
                f"  Emir ID: {order_id}, Yön: {side}, Fiyat: ${price}, Miktar: {qty}, Durum: {status}"
            )
...
def place_order_with_sl_tp(client: BinanceClient, symbol: str):
    ...
    try:
        ...
        logger.info(
            f"Ana emir oluşturuldu. Emir ID: {main_order['orderId']}, Durum: {main_order['status']}"
        )
        ...
        for order in open_orders:
            ...
            logger.info(
                f"  Emir ID: {order_id}, Yön: {side}, Tip: {order_type}, Stop Fiyatı: {stop_price}, Durum: {status}"
            )
