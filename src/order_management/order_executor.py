...
class OrderExecutor:
    ...
    def get_symbol_quantity(
        ...
        quantity = self.risk_manager.calculate_position_size(
            symbol=symbol, side=side, price=price, signal_data=signal_data
        )
    ...
    def create_market_buy_order(
        ...
        """
        Market alım emri oluşturur.

        Args:
            symbol: Trading sembolü
            quantity: İşlem miktarı (opsiyonel)
            signal_data: Sinyal verileri (opsiyonel)
        ...
        try:
            ...
            # Miktar belirtilmemişse minimum miktarı hesapla
            if quantity is None:
                ...
                quantity = float(
                    Decimal(str(min_qty * 1.1)).quantize(
                        Decimal(
                            "0."
                            + "0" * self.client.get_futures_quantity_precision(symbol)
                        ),
                        rounding=ROUND_DOWN,
                    )
                )
            ...
            if order["status"] == "FILLED":
                self._update_position(
                    symbol, "BUY", quantity, float(order.get("price", 0))
                )
    ...
    def create_market_sell_order(
        ...
        try:
            ...
            order = self.client.futures_create_order(
                ...
                type="MARKET",
                quantity=quantity,
                reduce_only=True,  # Pozisyonu kapatmak için
            )
            ...
            if order["status"] == "FILLED":
                self._update_position(
                    symbol, "SELL", quantity, float(order.get("price", 0))
                )
    ...
    def create_limit_buy_order(
        ...
        try:
            ...
            if not self.risk_manager.can_open_new_position(symbol, "BUY"):
                logger.warning(
                    f"{symbol} için alım pozisyonu açılamaz - risk limiti aşılmış"
                )
            ...
            if quantity is None:
                quantity = self.get_symbol_quantity(
                    symbol=symbol, side="BUY", price=price, signal_data=signal_data
                )
            ...
            self.orders.append(order_info)
            logger.info(
                f"LIMIT ALIM emri oluşturuldu: {symbol} - Fiyat: {price}, Miktar: {quantity}"
            )
        ...
        try:
            ...
            if quantity is None:
                ...
                if symbol in self.positions:
                    ...
                    quantity = self.get_symbol_quantity(
                        symbol=symbol, side="SELL", price=price, signal_data=signal_data
                    )
            ...
            self.orders.append(order_info)
            logger.info(
                f"LIMIT SATIM emri oluşturuldu: {symbol} - Fiyat: {price}, Miktar: {quantity}"
            )
            ...
            if side == "BUY":
                ...
                if symbol in self.positions:
                    ...
                    if quantity >= current_qty:
                        ...
                        logger.info(
                            f"{symbol} pozisyonu kısmen kapatıldı. Kalan: {current_qty - quantity}"
                        )
    ...
    def sync_positions_with_exchange(self) -> None:
        ...
        try:
            ...
            for symbol, position in list(self.positions.items()):
                ...
                base_asset = (
                    symbol.replace("USDT", "").replace("BUSD", "").replace("USDC", "")
                )
