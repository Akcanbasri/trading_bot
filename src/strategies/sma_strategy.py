...
class SMAStrategy:
    ...
    def generate_signal(self) -> Dict[str, Any]:
        ...
        """
        if self.historical_data is None or len(self.historical_data) < max(self.short_period, self.long_period):
            raise InsufficientDataError(
                f"Yetersiz veri: En az {max(self.short_period, self.long_period)} mum gerekli"
            )
        
        # SMA hesapla
        short_sma = self.historical_data['close'].rolling(window=self.short_period).mean()
        ...
        if short_sma.iloc[-2] <= long_sma.iloc[-2] and short_sma.iloc[-1] > long_sma.iloc[-1]:
            signal['action'] = 'BUY'
            signal['strength'] = (short_sma.iloc[-1] - long_sma.iloc[-1]) / long_sma.iloc[-1]
        elif short_sma.iloc[-2] >= long_sma.iloc[-2] and short_sma.iloc[-1] < long_sma.iloc[-1]:
            signal['action'] = 'SELL'
            signal['strength'] = (long_sma.iloc[-1] - short_sma.iloc[-1]) / long_sma.iloc[-1]
